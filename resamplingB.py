...
class Samples(ABC):
    ...
    def __init__(self, sample_data: np.ndarray, num_samples: int = 10000):
        ...
        self._confidence = 95.0
        self.confidence_interval = self.get_confidence_interval()           # confidence interval
    ...
    def get_confidence_interval(self) -> tuple:
        '''
        It returns a confidence% confident interval for a given statistic.
        '''
        alpha = 100 - self._confidence                
        lower_alpha = alpha / 2.0    
        lower_limit = np.percentile(self._sample_distribution, lower_alpha)
        upper_alpha = lower_alpha + self._confidence
        upper_limit = np.percentile(self._sample_distribution, upper_alpha)
...
class BootstrapIndependentHT(ResamplingHT):
    ...
    def get_sample_distribution_k(self) -> np.ndarray:    
        ...
        #sns.histplot(np.sqrt(sample_dist), stat='density', edgecolor='white', linewidth=1)          
    ...
    def confidence_intervals(self) -> np.ndarray:
        ...
        '''
        ci = np.array([self._sample_objects[k].confidence_interval for k in range(self._k)])
        return ci
